...
def test_is_ubuntu_returns_true_when_running_on_ubuntu(fs: FakeFilesystem) -> None:
...
def test_is_ubuntu_returns_false_when_not_running_on_ubuntu(fs: FakeFilesystem) -> None:
    """Non-Ubuntu systems are correctly identified."""
    fs.create_file("/etc/os-release", contents="NAME=Fedora\nID=fedora\n")
    assert not jar.is_ubuntu()


def test_is_ubuntu_returns_false_when_os_release_missing(fs: FakeFilesystem) -> None:
    """When /etc/os-release doesn't exist, is_ubuntu() returns False."""
    assert not jar.is_ubuntu()


def test_require_java_succeeds_when_java_is_available(monkeypatch: pytest.MonkeyPatch) -> None:
    """When java is on PATH, require_java() completes without error."""
    monkeypatch.setattr(jar.shutil, "which", lambda _: "/usr/bin/java")
    jar.require_java()


def test_require_java_shows_ubuntu_installation_instructions(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_require_java_shows_generic_error_on_non_ubuntu(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_bundled_jar_returns_valid_path() -> None:
...
def test_summary_file_or_tmp_file_uses_provided_path(tmp_path: Path) -> None:
...
def test_summary_file_or_tmp_file_creates_temp_file_when_none_provided() -> None:
...
def test_summary_file_or_tmp_file_cleans_up_temp_file_after_use() -> None:
...
def test_summary_file_or_tmp_file_preserves_provided_file_after_use(tmp_path: Path) -> None:
...
def test_run_jar_exits_on_dry_run_before_calling_subprocess(
    ...
    def mock_subprocess_run(*args: Any, **kwargs: Any) -> None:
